...
fig6_network_prepare_rf <- function(prepared, config = fig6_network_config(), impute = TRUE) {
  ...
  lag_columns <- paste0(unique(unlist(config$classes, use.names = FALSE)), "_lag0")
  fig6_network_columns(prepared, lag_columns)
...
fig6_network_fit_rf <- function(prepared_rf, outcome_label = "arg_abundance", config = fig6_network_config()) {
  ...
  if (config$ntree != 1000L || config$num.rep != 1000L) stop("Network RF configuration requires 1000 trees and 1000 permutations.", call. = FALSE)
  set.seed(config$seed)
  fit <- fig6_rf_permute(stats::reformulate(setdiff(names(prepared_rf), config$outcome), response = config$outcome),
...
fig6_prepare_hospital_rf <- function(total, group, dictionary, catalog) {
  ...
  fig6_hospital_assert(identical(predictor_ids, expected_predictors), "Hospital analysis requires the configured predictor order")
...
fig6_read_hospital_rf <- function(data_dir = "data/hospital_rf") {
  ...
  for (settings in list(out$total_settings, out$group_settings)) {
    fig6_hospital_assert(nrow(settings) == 1L && settings$trees == 1000L &&
                           settings$num.rep == 1000L && !settings$quick_test,
                         "Hospital RF results must use 1000 trees and 1000 permutations")
  }
  fig6_prepare_hospital_rf(out$total, out$group, out$dictionary, out$catalog)
  out
}
...
fig6_hospital_model_inputs <- function(data_dir = "data/hospital_rf") {
  ...
  key <- make_key(keys)
  fig6_hospital_assert(length(key) > 0L && !anyDuplicated(key), "Hospital-quarter input keys must be nonempty and unique")
  align <- function(x) {
    x_key <- make_key(x)
  ...
  fig6_hospital_assert(identical(names(raw), ids) && ncol(raw) == 105L && !anyNA(raw), "Predictor matrix must be complete and match the ordered dictionary")
  X <- raw
  ...
  fig6_hospital_assert(ncol(Y) == 22L && all(vapply(Y, function(y) all(is.finite(y)), logical(1))), "Expected 22 finite log10 ARG outcomes")
...
fig6_fit_hospital_rf <- function(model_inputs, dry_run = TRUE) {
  if (!exists("fig6_prepare_hospital_rf", mode = "function"))
    stop("Load the hospital RF helper functions before fitting", call. = FALSE)
  ...
  ids <- c(sprintf("DRUG__%03d", 1:97), "HOSP_mean_patient_days", "HOSP_mean_los",
           "HOSP_mean_discharges", "HOSP_mean_quarterly_amount", "QUARTER", "HOSPITAL", "CITY", "HOSP_type")
  check(is.data.frame(X) && nrow(X) > 0L && identical(names(X), ids) &&
          identical(as.character(dictionary$source_id), ids), "Predictors must match the ordered dictionary")
  ...
  check(nrow(model_inputs$keys) == nrow(X) && length(unique(model_inputs$keys$hospital_code)) > 0L, "Hospital-quarter keys must match the predictor rows")
  outcome_ids <- as.character(catalog$outcome_id)
  counts <- table(factor(catalog$outcome_level, levels = c("type", "clinical", "total")))
  check(is.data.frame(Y) && nrow(Y) == nrow(X) && ncol(Y) == 22L &&
  ...
  plan <- data.frame(dataset = rep("group22", length(outcome_ids)),
                     outcome_id = outcome_ids,
                     trees = 1000L, num.rep = 1000L, num.cores = 1L,
                     predictor_n = ncol(X), observation_n = nrow(X), stringsAsFactors = FALSE)
  ...
  fit_one <- function(task) {
    ...
    fit <- fig6_rf_permute(x = X, y = as.numeric(Y[[task$outcome_id]]),
      importance = TRUE, ntree = task$trees, num.rep = task$num.rep, num.cores = task$num.cores)
    ...
    result <- data.frame(dataset = task$dataset, outcome_id = task$outcome_id,
      ...
      num.rep = task$num.rep, trees = task$trees, model_seed = task$model_seed,
  ...
  group <- do.call(rbind, fitted)
  total <- group[group$outcome_id == "TOTAL__all_ARG", , drop = FALSE]
...
fig6_hospital_validate_family <- function(d, planned_n) {
  ...
  if (any(!is.finite(d$n[ok]) | !is.finite(d$hospital_n[ok]) | d$n[ok] <= 0 | d$hospital_n[ok] <= 0 | d$hospital_n[ok] > d$n[ok])) stop("Invalid observation or hospital counts in fitted results", call. = FALSE)
  ...
  if (any(!is.finite(d$fdr_global_profile_dataset[ok])) ||
    ...
    stop("Saved adjusted P values differ from BH over the full planned family", call. = FALSE)
...
fig6_hospital_read_results <- function(full_results_dir) {
  ...
  for (i in seq_len(nrow(families))) {
    ...
    if (any(!is.na(d$aggregation) & !grepl("log10\\(x\\+1e-08\\)",d$aggregation)))
      stop("Outcome transformation differs from log10(x+1e-08)",call.=FALSE)
    ok <- d$status=="success" & is.finite(d$p_within)
...
fig6_hospital_plots <- function(d, family="Arial") {
  ...
  fig6_hospital_columns(d, c("beta_within", "ci_low", "ci_high", "q", "profile", "order_key",
                             "section", "panel", "outcome", "drug"), "Hospital plot selection")
  if (anyNA(d[c("beta_within", "ci_low", "ci_high", "q", "profile", "order_key", "section", "panel")]) ||
      any(!is.finite(d$beta_within) | !is.finite(d$ci_low) | !is.finite(d$ci_high)) ||
      any(d$ci_low > d$beta_within | d$ci_high < d$beta_within | d$q < 0 | d$q > 1) ||
      any(!d$profile %in% c("oral", "injection", "combined")))
    stop("Hospital plot estimates, intervals, routes and BH-adjusted P values must be valid.", call.=FALSE)
  ...
  if (!identical(c(length(unique(go$order_key)),length(unique(gc$order_key)),length(unique(gi$order_key)),length(unique(h$order_key))),c(4L,16L,19L,12L)))
    stop("Selection does not match the configured panel layout",call.=FALSE)

  gl<-c(min(-.60,d$low[d$panel=="g"]-.04),max(1.08,d$high[d$panel=="g"]+.04,d$star_y[d$panel=="g"&d$star=="*"]+.09));gb<-c(-.5,0,.5,1)
  hl<-c(min(-.15,h$low-.04),max(.85,h$high+.04,h$star_y[h$star=="*"]+.04))
  g <- cowplot::ggdraw()+
  ...
  hp <- cowplot::ggdraw()+
    cowplot::draw_plot(panel(h,12,hl,c(0,.2,.4,.6,.8)),x=.0491,y=.2717,width=.9509,height=.7083)+
...
fig6_hospital_model_frame <- function(outcomes,exposure,metadata,outcome_id,predictor_id,
  ...
  if(!is.null(expected_n) && length(km)!=expected_n) stop("Unexpected hospital-quarter count",call.=FALSE)
  if(!is.null(expected_hospitals) && length(unique(metadata$hospital_code))!=expected_hospitals) stop("Unexpected hospital count",call.=FALSE)
  ...
  if(!is.numeric(y)||!is.numeric(xr)||!is.numeric(patient)||any(is.infinite(c(y,xr,patient))))
    stop("Outcome, exposure and patient-days values must be numeric and finite when observed",call.=FALSE)
  if(any(xr < 0, na.rm=TRUE)||any(patient < 0, na.rm=TRUE))
    stop("Consumption and patient-days must be nonnegative",call.=FALSE)
  if(outcome_scale=="linear"){
    if(any(y<0,na.rm=TRUE))stop("Linear outcome abundance must be non-negative",call.=FALSE)
    y<-log10(y+1e-8)
  }
  hospital<-factor(as.character(metadata$hospital_code))
  xl<-log1p(xr)
  xb<-ave(xl,hospital,FUN=function(v)mean(v,na.rm=TRUE));xw<-xl-xb
  out<-data.frame(y=as.numeric(y),hospital=hospital,quarter_f=factor(metadata$quarter),
    patient_z=fig6_hospital_fit_z(log1p(patient)),
    drug_within_z=fig6_hospital_fit_z(xw),drug_between_z=fig6_hospital_fit_z(xb),
    exposure_nonnegative=xr,row.names=NULL)
...
fig6_fit_hospital_lmm <- function(outcomes,exposures,metadata,dataset=c("full2646","group22"),
                                 ...
                                 expected_n=NULL,expected_hospitals=NULL,min_rows=30L,
                                 return_models=FALSE) {
  dataset<-as.character(dataset)[1];outcome_scale<-match.arg(outcome_scale)
  if(is.null(expected_n)) expected_n <- nrow(metadata)
  if(is.null(expected_hospitals)) expected_hospitals <- length(unique(metadata$hospital_code))
  ...
  for(profile in names(exposures)){
    ...
    for(oid in outcome_ids)for(did in predictor_ids){
      ...
      if(nrow(d)<min_rows||nlevels(droplevels(d$hospital))<5L)row$reason<-"insufficient_rows"
      else if(!any(d$exposure_nonnegative>0))row$reason<-"structural_zero"
...
fig6_rf_geometry <- function(table, class_table=NULL) {
  ...
  label_values <- if ('Relative_unclipped' %in% names(d)) d$Relative_unclipped else d$Importance
  if (!is.numeric(label_values) || any(!is.finite(label_values)))
    stop('Predictor label values must be finite numeric values.')
  d$ValueLabel <- paste0(sprintf('%.1f%%',label_values),d$Stars)
