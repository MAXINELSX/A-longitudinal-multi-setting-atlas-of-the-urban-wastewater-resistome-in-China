...
prepare_fig4c_data <- function(input_file) {
  ...
  if (!nrow(df)) stop("Input must contain at least one cluster.", call. = FALSE)
...
validate_pairs <- function(df) {
  ...
  if (!nrow(df)) stop("Input must contain at least one pair.")
  ...
  if (any(as.numeric(df$ANI_min) < 0 | as.numeric(df$ANI_min) > 100)) {
    stop("ANI_min must be a percentage between zero and 100")
  }
  if (any(as.numeric(df$callable_fraction) < 0 | as.numeric(df$callable_fraction) > 1)) {
    stop("Callable fraction must lie between zero and one")
  }
...
draw_fig4d <- function(df, legend_position = c("bottom", "right")) {
  ...
  if (!nrow(df)) stop("No pairs available for plotting.")
  ...
  snp_min <- min(7.5, min(df$fixed_SNP_Mbp))
  snp_max <- max(1000, max(df$fixed_SNP_Mbp))
  snp_x <- function(value) {
    x$snp_left + (log10(value) - log10(snp_min)) /
      (log10(snp_max) - log10(snp_min)) * (x$snp_right - x$snp_left)
  }
  callable_min <- min(0.5, min(df$callable_fraction))
  callable_max <- max(0.9, max(df$callable_fraction))
  callable_size <- function(value) 2.6 + (value - callable_min) /
    (callable_max - callable_min) * 3.5
  ...
  snp_ticks <- 10^seq(ceiling(log10(snp_min)), floor(log10(snp_max)))
  for (tick in snp_ticks) {
    tick_x <- snp_x(tick)
    grid::grid.lines(
...
prepare_fig4e <- function(data_dir) {
  ...
  if (nrow(displayed) != length(expected_keys) || anyDuplicated(actual_keys) ||
...
run_fig4e <- function(data_dir, output_dir) {
  ...
  context_limit <- max(6300, max(d$context$Contigs) * 1.1)
  context_breaks <- if (context_limit == 6300) c(0, 2000, 4000, 6000) else pretty(c(0, context_limit), n = 4)
  p_context <- ggplot2::ggplot(d$context, ggplot2::aes(Contigs, Context_plot)) +
    ...
    ggplot2::scale_x_continuous(labels = comma, breaks = context_breaks,
                               limits = c(0, context_limit), expand = ggplot2::expansion(mult = c(0, 0))) +
  ...
  taxa_limit <- max(3300, max(d$taxa$Contigs) * 1.1)
  taxa_breaks <- if (taxa_limit == 3300) c(0, 1000, 2000, 3000) else pretty(c(0, taxa_limit), n = 4)
  p_taxonomy <- ggplot2::ggplot(d$taxa, ggplot2::aes(Contigs, Taxon_plot)) +
    ggplot2::geom_col(ggplot2::aes(fill = fill), width = 0.56) +
    ggplot2::geom_text(ggplot2::aes(label = label, hjust = hjust, colour = label_colour),
                       size = 2.8, family = "Arial", fontface = "bold") +
    ggplot2::scale_fill_identity() + ggplot2::scale_colour_identity() +
    ggplot2::scale_x_continuous(labels = comma, breaks = taxa_breaks,
                               limits = c(0, taxa_limit), expand = ggplot2::expansion(mult = c(0, 0))) +
