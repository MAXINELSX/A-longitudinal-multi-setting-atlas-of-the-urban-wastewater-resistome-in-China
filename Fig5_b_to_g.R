# Figure 5 b-g: complete, self-contained script.
...
prepare_fig5_data <- function(long, meta, expected_samples=2614L, expected_subtypes=102L) {
...
read_fig5_data <- function(data_root, metadata_file, audit_dir=NULL) {
  ...
  suffix <- '_split_by_12city_removed_target_columns_leftjoin_merged_subtype_cleaned_filtered_target_subtypes.xlsx'
  ...
  stopifnot(length(unique(result$meta$PhysicalSite))==130L,
            length(unique(result$meta$City))==12L,
            length(unique(result$meta$SamplingMonth))==14L)
  ...
  if(!is.null(audit_dir)) {
    ...
    writeLines('Author-confirmed encoding: blank subtype abundance cells and numeric zeros denote nondetection. Missing sample metadata are not imputed.',file.path(audit_dir,'blank_policy.txt'))
  }
  result
}
...
# ===== Fig5_bc.R =====
# Fig. 5b/c: portable recovery of the original alluvial and four-set Venn.
...
validate_fig5_bc_table <- function(x, scope=c("risk", "tier_i")) {
  ...
  expected <- if(scope=="risk") c(102L,156L,133L,35L) else c(102L,0L,0L,0L)
  got <- as.integer(table(factor(x$risk_level, levels=fig5_bc_levels)))
  if(!identical(got, expected)) stop("Tier roster mismatch: expected ", paste(expected, collapse="/"), "; observed ", paste(got,collapse="/"))
...
read_fig5_bc_inputs <- function(data_dir) {
  ...
  if(!identical(as.integer(counts),c(639L,45L,594L,168L,426L))) stop("Core/contig/eligible provenance counts disagree with the audited set")
  if(sum(tier_i$n_settings==4)!=73 || sum(!tier_i$Hospital)!=10 || sum(tier_i$Hospital & tier_i$n_settings==1)!=6) stop("Tier I core overlap disagrees with 73 shared / 10 outside-hospital / 6 hospital-only")
  expected_hospital <- paste0("beta_lactam__",c("KPC-2","NDM-1","IMP-1","IMP-4","IMP-8","VIM-2"))
  if(!setequal(tier_i$arg_subtype[tier_i$Hospital & tier_i$n_settings==1],expected_hospital)) stop("Hospital-only Tier I identity mismatch")
...
run_fig5_bc <- function(data_dir, out_dir) {
  ...
  p_b <- ggplot2::ggplot(x$flow,ggplot2::aes(axis1=type_display,axis2=risk_level,axis3=setting,y=n_subtypes)) +
    ...
    ggplot2::labs(x=NULL,y="Subtype-setting occurrences",fill="Prioritization tier",caption="A subtype contributes once for each setting in which it is core. Unique tier counts: 102, 156, 133 and 35.") +
  ...
  # Keep the original Venn geometry, placing set labels clear of region counts.
...
run_fig5d <- function(long, meta, output_dir, ellipse_level = 0.95,
  ...
  if (compute_dispersion) {
    message("Fig5d: running optional PERMDISP; main outputs are already available.")
...
# Plot from the exported scores and test results without recomputing distances
# or permutations. This also allows quick cosmetic revisions of a verified run.
fig5d_plot <- function(plot_data, statistics, axis_percent, ellipse_level = 0.95) {
  fig5d_require_packages()
  plot_data$Setting <- factor(plot_data$Setting,
                              levels = c("Community", "Hospital", "Wet market", "WWTP"))
  # ggplot2's normal ellipse needs at least four points and full-rank covariance.
  ...
  # A 95% data ellipse may extend beyond every observed point. Include its
  # actual plotted coordinates when setting limits, so it stays on the canvas.
...
# ===== Fig5_models.R =====
# Fig. 5e Tier I abundance LMM; supplementary Tier I richness NB model.
# Reuses the verified Fig. 2 repeated-site model and contrast implementation.
# abundance: original-scale Gaussian LMM (REML), not log transformed.
# richness: negative-binomial mixed model (log link), integer detections.
# Both: setting + city + sampling month fixed; physical-site random intercept.
# Inference: two-sided asymptotic Wald z, BH across six setting comparisons.
# Numerical labels on plots are raw arithmetic means, not adjusted means.
...
fig5_fg_months <- function() {
  format(seq(as.Date("2024-11-01"), as.Date("2025-12-01"), by = "month"), "%Y-%m")
}
...
run_fig5 <- function(data_root, metadata_file, annotation_dir,
  ...
  if (needs_samples) {
    cohort <- if (is.null(prepared_data)) {
      ...
      if (length(unique(checked$meta$PhysicalSite)) != 130L ||
          length(unique(checked$meta$City)) != 12L ||
          length(unique(checked$meta$SamplingMonth)) != 14L)
        stop("Prepared cohort differs from the intended 130-site, 12-city, 14-month network.")
      checked
    }
    if (!setequal(cohort$subtypes, tier$tier_i$arg_subtype))
      stop("The abundance matrix is not the exact 102-subtype Tier I roster used in b/c.")
  ...
  if ("d" %in% panels)
    result$d <- run_fig5d(cohort$long, cohort$meta, file.path(output_dir,"d_PCoA"),
      ellipse_level=.95, permutations=999L, seed=123L, zero_policy="error")
  if ("e" %in% panels)
