# Supplementary Figures S2-S14 | standalone code bundle | 2026-09-19
...
fig6_network_model_spec <- function(data, class, lag, config = fig6_network_config()) {
  ...
  if (!setequal(expected, predictors)) stop("Missing required class predictors for ", class, "/", lag, ": ", paste(setdiff(expected, predictors), collapse = ", "), call. = FALSE)
...
# ===== S2_S4_S5.R =====
# S2/S4/S5. Source-inert. Metadata fields: sample_id, city, setting, month,
# physical_site. Abundance matrices: sample rows, subtype/group columns.
...
si245_site_means <- function(abundance, metadata) {
  ...
  a <- t(vapply(sites, function(id) colMeans(z$a[z$m$physical_site == id, , drop = FALSE]), numeric(ncol(z$a))))
...
# Original arithmetic means remain descriptive; letters come from the model.
si245_nb <- function(d) {
...
si245_s5_tests <- function(abundance, metadata, seed = 123L) {
  ...
  for (setting in unique(m$setting)) for (feature in colnames(a)) {
    ...
    rows[[index]] <- do.call(rbind, lapply(unique(city), function(target) {
      ...
      P <- stats::fisher.test(matrix(c(aa, bb, cc, dd), nrow = 2, byrow = TRUE), alternative = "greater")$p.value
      data.frame(Setting = setting, Feature = feature, City = target, a = aa, b = bb, c = cc, d = dd,
        ...
        Mean_abundance = mean(x[inside]), P = P)
    }))
  }
  if (!length(rows)) {
    empty <- data.frame(Setting = character(), Feature = character(), City = character(),
      ...
      Rate = numeric(), Other_rate = numeric(), Ratio = numeric(), Mean_abundance = numeric(), P = numeric(),
  ...
  # Preserve original comparison order when joining global test results.
  ...
  choose <- function(d) {
    ...
    if (any(supported)) {
      ...
      ord <- order(-match(d$Evidence, c("Low", "Moderate", "High")), -ifelse(is.infinite(d$Ratio), 999, d$Ratio), -d$Rate, -d$Events)
    } else if (any(d$Enriched)) {
      d <- d[d$Enriched, , drop = FALSE]; ord <- order(-ifelse(is.infinite(d$Ratio), 999, d$Ratio), -d$Rate, -d$Events)
    } else ord <- order(-d$Rate, -d$Events, -d$Mean_abundance)
    d[ord[1], , drop = FALSE]
  }
...
si245_s5 <- function(group_abundance, metadata, seed = 123L, site_sensitivity = TRUE) {
  primary <- si245_s5_tests(group_abundance, metadata, seed)
  result <- list(primary = primary)
  if (site_sensitivity) {
    site <- si245_site_means(group_abundance, metadata)
    sensitivity <- si245_s5_tests(site$a, site$m, seed)
    selected <- primary$winners[primary$winners$Evidence %in% c("High", "Moderate"), , drop = FALSE]
    key <- c("Setting", "Feature", "City")
    matched <- merge(selected, sensitivity$comparisons, by = key, all.x = TRUE, suffixes = c("_sample", "_site"), sort = FALSE)
    matched$Same_direction <- ifelse(is.na(matched$Ratio_site), NA, matched$Ratio_site > 1)
    matched$Site_BH_significant <- ifelse(is.na(matched$p_enriched_FDR_within_feature_site), NA,
      matched$p_enriched_FDR_within_feature_site < .05)
    result$site <- sensitivity; result$selected_sensitivity <- matched
  }
  result
}
...
si_root_input <- function(abundance,metadata) {
  # abundance: complete numeric sample x subtype matrix in copies/cell.
  # Blank subtype cells already confirmed as non-detections must be converted
  # to zero on import; an unavailable measurement must not be imputed as zero.
...
si6_select_variants <- function(all_setting_stats,top1_setting,top1_city,n_major=8L) {
  # Recovered Aug17 display rule, not a newly fitted significance filter:
  # all setting/city representatives + top8 maximum setting composition.
  ...
  for(i in seq_along(inputs)) if(!all(needs[[i]] %in% names(inputs[[i]]))) stop('Incomplete original S6 selection table.')
...
si6_family_bars <- function(abundance,metadata,variant_map,selected_variants,
  ...
  # variant_map: Subtype, Family, Variant; explicit mapping avoids substring hits.
  # selected_variants: Family, Variant from si6_select_variants or original table.
  si_root_require(c('dplyr','tidyr','ggplot2'))
  ...
  if(!all(c('Family','Variant') %in% names(selected_variants))) stop('Supply the original selected variants.')
...
si14_profile_features <- function(abundance,metadata,marker_map,tier1_subtypes,bin_days=14L) {
  # marker_map: Marker, Subtype. Supply the original NNLS marker dictionary;
  # its OXA scope must not be replaced silently with the Fig2 OXA-48 marker.
  inp<-si_root_input(abundance,metadata);a<-inp$abundance;m<-inp$metadata
  si_root_require('dplyr')
...
si14_plot <- function(coefficients) {
  ...
  # Cross-hatching spans the whole 100% bar and never removes its coefficients.
  segments<-list();n<-0L
  for(i in seq_len(nrow(bad))) for(direction in c(-1,1)) for(offset in seq(-.15,1.15,by=.055)) {
    xs<-c(-.4,.4);ys<-offset+direction*.24*xs
    # Clip each diagonal analytically to y in [0,1].
...
# Typed schemas. All identifiers are character, not numeric. Unknown host species
# may be NA; host_rank is Kraken's final rank (S means species). Unknown carrier is
# explicitly 'unknown'. Each annotation row represents ONE ORF best-hit occurrence.
# Metadata must be an authoritative sample registry; physical_site is never
# fabricated from core group, ARG subtype, setting, or an aggregate input table.
si789_schemas <- function() list(
  ...
  s8_model = c(sample_id = "character", arg_subtype = "character", metric = "character", value = "numeric"),
...
# Length-prefixed keys avoid collisions even when IDs contain punctuation.
...
# S7 uses unique sample-contig identifiers within each ARG type. Multiple ORFs of
# one type collapse; a multi-type contig contributes once to each applicable type.
# Unknown carriers cause an error because S7's three compartments are exhaustive.
...
# Core membership is supplied explicitly for ALL four settings per subtype,
# including FALSE cells. These are membership flags, not simple detection flags.
...
# Optional membership builder from a COMPLETE nonnegative sample x subtype matrix.
# Rows include measured zero abundances; missing values never mean zero. Column
# names are subtype IDs and row names match every sample in metadata exactly.
...
# Six metrics per observed subtype. All mobility denominators are ALL unique
# subtype-carrying contigs, following manuscript 4.9 (unknown carriers retained).
# Only S-rank assigned contigs enter host fractions/diversity; richness is zero
# when none are assigned, while dominant fraction and Shannon are undefined (NA).
si789_metric_values <- function(units) {
  ...
  c(dominant_host_fraction = if (length(p)) max(p) else NA_real_,
    ...
    mobile_fraction = mean(units$carrier %in% c("plasmid", "phage")),
    plasmid_fraction = mean(units$carrier == "plasmid"), phage_fraction = mean(units$carrier == "phage"))
}
...
si789_prepare_s8 <- function(annotations, metadata, core_membership) {
  ...
  model <- summarize(c("sample_id", "arg_subtype"))
  plot <- summarize("arg_subtype") # pooled contig summaries, NOT means of sample metrics
  ...
  list(model_input = model, plot_data = plot, core_groups = groups, metadata = si789_metadata(metadata),
       audit = data.frame(annotation_occurrences = nrow(a), subtype_contig_units = nrow(units),
                          ...
                          mobility_denominator = "All subtype-carrying contigs",
                          host_denominator = "Species-rank assigned subtype-carrying contigs"))
}

# Adapter for the actual supplied E:/c_merge/2614/core_arg_trait_table.tsv.
# It is a PLOT-only table. The *_all fields implement manuscript 4.9; *_known
# fields from the legacy Dunn plot must not silently replace them.
...
# Mutual exclusivity and provenance are mandatory. This module does not invent a
# clinical list or substitute the ESKAPEE highlight set used in Fig. 3a.
...
si789_prepare_s9 <- function(annotations, metadata, host_sets) {
  ...
  # NO subtype deduplication: two different ORFs of the same subtype survive.
  occurrences <- species
...
# Shared Gaussian LMM inference follows the already audited Fig3c module:
# REML, original response scale, two-sided asymptotic Wald z contrasts, BH within
# each outcome. No arbitrary random intercept for ARG subtype is introduced.
si789_lmm <- function(d, focal, levels, expected_pairs) {
  ...
  if (any(vapply(d[fixed], nlevels, integer(1)) < 2L)) stop("Fixed effect has fewer than two observed levels; do not silently alter the manuscript model.")
...
si789_fit_s8 <- function(model_input, metadata, core_membership) {
  # An aggregate trait table fails here because it lacks sample_id. Never create
  # mock sample/site IDs to make such a table fit the repeated-site model.
  si789_check(model_input, si789_schemas()$s8_model, "sample-level S8 model input", "value")
  si789_validate_s8_values(model_input)
  if (!setequal(model_input$metric, si789_metrics())) stop("S8 model input requires all six metrics.")
  if (anyDuplicated(si789_key(model_input, c("sample_id", "arg_subtype", "metric"))))
    stop("S8 requires one observation per sample, subtype and metric.")
  m <- si789_metadata(metadata); groups <- si789_core_groups(core_membership); d <- model_input
  i <- match(d$sample_id, m$sample_id); j <- match(d$arg_subtype, groups$arg_subtype)
  if (anyNA(i) || anyNA(j)) stop("Missing sample metadata or subtype core membership.")
  for (key in setdiff(names(si789_schemas()$metadata), "sample_id")) d[[key]] <- m[[key]][i]
  d$core_group <- groups$core_group[j]
  if (!any(table(unique(d[c("sample_id", "physical_site")])$physical_site) > 1L))
    stop("S8 requires repeated sampled physical sites; aggregate subtype tables are not model inputs.")
  result <- lapply(si789_metrics(), function(metric) {
    q <- d[d$metric == metric & !is.na(d$value), , drop = FALSE]
    r <- si789_lmm(q, "core_group", si789_groups(), 3L)
    r$diagnostics$metric <- metric
    r$diagnostics$missing_observations <- sum(d$metric == metric & is.na(d$value))
    r$diagnostics$transformation_note <- "S2-5 specifies no response transformation; identity used; richness log10(x+1) is display-only."
    r
  })
  names(result) <- si789_metrics()
  list(results = result, model_input = d,
       excluded_missing = d[is.na(d$value), c("sample_id", "arg_subtype", "metric"), drop = FALSE])
}
...
# Half-violin polygons use a density bounded to the observed range. Jitter has an
# explicit seed, so no global RNG change is needed. Boxplot includes all points.
...
si789_plot_s8 <- function(plot_data, fitted = NULL, richness_display = c("original", "log10p1")) {
...
# Explicit optional input reader. Does not discover private files or guess sheets.
...
# S11a,b INPUT: one matched hospital-quarter row, with city, hospital, quarter,
# patient_days and discharges. Hospital identifiers must be globally unique.
# The 43-row matched analytical registry must be used, not all pharmacy rows.
# Each hospital-quarter receives equal weight; this is NOT patient-day weighting.
si111213_s11_activity <- function(activity,
    expected = c(hospital_quarters = 43L, hospitals = 15L, cities = 8L)) {
  ...
  if (!is.null(expected) && any(observed[names(expected)] != expected))
    stop("S11 matched cohort differs from the declared 43 hospital-quarters/15 hospitals/8 cities. ",
         "Use expected=NULL only for an explicitly different cohort or synthetic test.", call. = FALSE)
...
# Optional raw-DDD adapter. INPUT ddd: city, hospital, quarter, route, category, ddd.
# route must be oral/injection. Rows may represent separate drugs; summing within
# category happens before intensity calculation. Missing DDD amounts propagate.
# Explicit zero rows are necessary for confirmed zero use; absent rows stay absent.
# Combined route is computed only when BOTH routes were observed for that category.
...
# S11c-e INPUT: one category/route/hospital/quarter intensity in DDDs/100 patient-days.
# Retains explicit zero intensities. Never fills absent or NA values with zero.
# First average valid quarters within each hospital; then equally average hospitals
# within city/category/route. Category contributions are stacked; bar label is sum.
...
# Exact published-panel selection recovered from source vars_fig3/vars_fig4,
# not statistical filtering or a newly chosen top-predictor threshold.
si111213_config <- function() {
  ...
  config$selection_provenance <- "Recovered vars_fig3 and vars_fig4 in WWTP total and Tier I sections of 修改LMM+RF-20260526.R"
  config
}

# INPUT quality: city/date/Index/value, possibly multiple site measurements on a
# city-date. Canonical indices are CODCr, NH3_N, TSS, pH, flow,
# wastewater_temperature. Flow=0 is missing; wastewater_temperature=0 is retained.
...
# Match exact source boundaries: nearest dated record at delta 0..90 for lag0;
# inclusive [date-window,date] means for lag1/2/3. A latest recorded NA stays NA,
# rather than silently replacing it with an earlier record. No coverage threshold.
...
# INPUT samples: sample_id,city,site_nm,setting,sample_date,season,arg_abundance.
# Outcome must already be summed per sample for total OR Tier I; call separately.
# daily: city/date/Index/value, one row per city/date/index. Context year-month
# derives from sample_date. Physical site IDs are nested in city for uniqueness.
si111213_prepare_wwtp <- function(samples, daily, quality, config = si111213_config()) {
  ...
  # Unlike the old source, missing outcomes are excluded, never changed to 1e-8.
...
# RF-only kNN branch. Exact model columns prevent flow, temperature, site ID or
# old imputation-indicator columns from entering the forest accidentally.
...
# S12 fit entry points: these are NEVER invoked by source().
...
si111213_fit_wwtp_rf <- function(prepared_rf, outcome_label, config = si111213_config()) {
  ...
  if (!"num.rep" %in% names(formals(getS3method("rfPermute", "default", envir = asNamespace("rfPermute")))))
    stop("rfPermute API must support num.rep; nrep is not an interchangeable argument.", call. = FALSE)
  set.seed(config$seed)
  fit <- rfPermute::rfPermute(stats::reformulate(setdiff(names(prepared_rf), config$outcome), config$outcome),
...
# Signed normalization follows current S5-1 and supersedes the old plot's clipping.
...
si111213_rf_classes <- function(table) {
  ...
  out <- stats::aggregate(table["Importance"], table["Class"], sum)
  # No clipping, absolute-value substitution, or positive-only re-normalization.
  names(out)[2] <- "Percent"
  out
}

# Model results are complete predictor x lag tables after BH, not panel subsets.
# The split retains the original fitted values and Q values without re-adjustment.
...
# Equal angular widths per predictor; outer sectors span constituent predictors.
# Importance remains signed, shown numerically and as bars around a zero radius.
# Sector widths represent predictor counts, never importance.
si111213_plot_rf <- function(table, title = NULL) {
  ...
  d$id <- seq_len(nrow(d))
  span <- max(abs(d$Importance))
  if (!is.finite(span) || span == 0) stop("Cannot plot empty importance variation.", call. = FALSE)
  baseline <- max(0, -min(d$Importance)) + .25 * span
  d$ymin <- pmin(baseline, baseline + d$Importance)
  d$ymax <- pmax(baseline, baseline + d$Importance)
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::labs(title = title, caption = "Dashed circle: zero predictor importance. Negative percentages are retained.") +
...
# Explicit artifact export; no files are written until this function is called.
...
# ===== SI_entry.R =====
# Convenience utilities. Sourcing the complete script does not run analyses.
...
# Run only after explicitly deciding to install dependencies on this machine.
...
# Canonical metadata for count/composition and repeated-site panels. Supplied
# columns must already identify actual physical sites, not setting labels.
...
# Explicit output only: each list element must be a ggplot or patchwork object.
