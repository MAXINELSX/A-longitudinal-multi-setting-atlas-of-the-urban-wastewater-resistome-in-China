if (.Platform$OS.type == "windows") {
  invisible(try(suppressWarnings(Sys.setlocale("LC_ALL", "English_United States.utf8")), silent = TRUE))
}
set.seed(20260918)
...
resolve_abundance_missing <- function(x, policy='stop') {
  ...
  if (!policy %in% c('stop','confirmed_nondetection')) stop('Unknown missing-value policy.')
  ...
  if (policy == 'confirmed_nondetection') x[is.na(x)] <- 0
  x
}

load_fig2_cohort <- function(data_root, audit_dir, blank_policy='confirmed_nondetection',
                             expected_samples=NULL, expected_subtypes=NULL,
                             expected_sites=NULL, expected_cities=NULL) {
  dir.create(audit_dir, recursive=TRUE, showWarnings=FALSE)
  suffix <- Sys.getenv('FIG2_SUBTYPE_SUFFIX', '.xlsx')
  chinese <- c(Hospital='\u533b\u9662', WWTP='\u6c61\u6c34\u5904\u7406\u5382', Community='\u793e\u533a', 'Wet market'='\u519c\u96c6\u8d38\u5e02\u573a')
  metadata_path <- Sys.getenv('FIG2_METADATA_FILE', file.path(data_root, 'sample_metadata.xlsx'))
  ...
  stopifnot(!anyNA(meta), nrow(meta)>0L, !'SH' %in% meta$City)
  if (!is.null(expected_samples)) stopifnot(nrow(meta)==expected_samples)
  if (!is.null(expected_sites)) stopifnot(length(unique(meta$PhysicalSite))==expected_sites)
  if (!is.null(expected_cities)) stopifnot(length(unique(meta$City))==expected_cities)
  ...
  for (setting in names(chinese)) {
    ...
    for (sheet in readxl::excel_sheets(path)) {
      ...
      stopifnot(setequal(subtype,ref_subtypes))
      if (!is.null(expected_subtypes)) stopifnot(length(subtype)==expected_subtypes)
      ...
      x <- resolve_abundance_missing(x, if(blank_policy=='stop') 'confirmed_nondetection' else blank_policy)
  ...
  utils::write.csv(data.frame(Blank_policy=blank_policy,Blank_cells=nrow(blanks),
    Affected_samples=length(unique(blanks$Sample)),Status=if(blank_policy=='confirmed_nondetection')
        'Blank subtype cells and explicit numeric zeros denote nondetection'
      else blank_policy),
  ...
  stopifnot(setequal(samples_seen,meta$Sample),nrow(long)==nrow(meta)*length(ref_subtypes))
...
audited_global_input <- function(df) {
  ...
  if (duplicate_policy == "exploratory" && any(duplicate_extra)) {
    ...
    if (duplicate_policy == "drop") note <- paste(note,
      paste0(sum(duplicate_extra), " exact duplicate extra external records removed; first occurrences retained."))
    message(note)
  }
...
run_panel_d <- function() {
  ...
  make_index_plot <- function(
    ...
    letter_df <- make_sig_letters(pairwise_res, group_order = present_order, alpha = 0.05) %>%
      mutate(Group = factor(Group, levels = group_order),
             x_id = match(as.character(Group), group_order))
...
in_dir <- FIG2_DATA_ROOT
out_dir <- file.path(.figure_results_dir, "panels_b_c")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

file_list <- c(
  "Hospital"   = file.path(in_dir, "subtype_by_site_\u533b\u9662.xlsx"),
  "WWTP"       = file.path(in_dir, "subtype_by_site_\u6c61\u6c34\u5904\u7406\u5382.xlsx"),
  "Community"  = file.path(in_dir, "subtype_by_site_\u793e\u533a.xlsx"),
  "Wet market" = file.path(in_dir, "subtype_by_site_\u519c\u96c6\u8d38\u5e02\u573a.xlsx")
)
...
audited_panel_f_plots <- function(tables) {
  ...
  y_map <- stats::setNames(1:4, .f_settings)
...
audited_panel_f_export <- function(tables, output_dir, make_plots = TRUE,
  ...
  definitions <- data.frame(
    ...
    Definition = c(
      ...
      "No dendrogram is drawn in this layout.",
...
run_panel_f_original <- function(long = .canonical_long, meta = .canonical_meta,
  ...
  if (!exists("audited_panel_f_core", mode = "function")) stop("The core-summary function is unavailable; source the complete script.")
  ...
  writeLines(c(
    ...
    "Grouped circular tree layout uses Ward.D2 clustering and a four-cluster cut.",
    "Constant tree feature columns scale to zero.",
    ...
    "The UpSet left bars and membership rows share one y map.",
  ...
  p_circle <- p_circle + labs(caption = paste0("All ", nrow(df2), " core subtypes; ", nrow(type_summary), " ARG types. Descriptive clustering."))
...
run_panels_gh <- function(
  ...
  if (nrow(meta) == 0L || anyNA(meta) || anyDuplicated(meta$Sample) ||
      !setequal(meta$City, cities) || !setequal(meta$Setting, settings)) {
    stop("Fig. 2g/h require complete metadata with the configured cities and settings.")
  }
  meta$YearMonth <- format(meta$Sample_Date, "%Y-%m")
  month_dates <- as.Date(paste0(meta$YearMonth, "-01"))
  months <- format(seq(min(month_dates), max(month_dates), by = "month"), "%Y-%m")
  if (!is.numeric(canonical_long$Abundance) || anyNA(canonical_long$Abundance) ||
      any(!is.finite(canonical_long$Abundance)) || any(canonical_long$Abundance < 0)) {
  ...
  if (nrow(detection) != nrow(specs)*length(settings) || any(detection$n_sample_rows != detection$n_samples)) {
    stop("Fig. 2g requires every marker/setting combination with complete setting denominators.")
  }
  ...
  h_sample <- clinical |>
    ...
    dplyr::left_join(meta, by = "Sample")
  if (nrow(h_sample) != nrow(meta)*nrow(targets) || anyNA(h_sample)) stop("Fig. 2h sample/gene table is incomplete.")
  ...
  if (nrow(monthly_counts) != length(months)*length(settings) ||
      nrow(h_monthly) != length(months)*length(settings)*nrow(targets)) {
  ...
  message("Fig. 2g/h outputs saved.")
  invisible(list(
...
FIG2_DATA_ROOT <- Sys.getenv('FIG2_DATA_ROOT','data/fig2')
...
run_fig2 <- function() {
  ...
  results <- Sys.getenv('ARG_RESULTS_DIR',file.path(base,'Fig2_results'))
  dir.create(results,recursive=TRUE,showWarnings=FALSE)
  results <- normalizePath(results,winslash='/')
  .figure_results_dir <<- results
  .external_data_root <<- Sys.getenv('ARG_EXTERNAL_DATA_ROOT','data')
  ...
  writeLines(c(paste('Blank policy:',BLANK_POLICY),paste('2c model:',C_MODEL),
    ...
    'Global comparisons use exploratory Kruskal-Wallis/Dunn tests; exact duplicate extra external records are removed by default.',
    'Monthly profiles are descriptive; sample/site composition is not constant.',
    if(BLANK_POLICY=='confirmed_nondetection')
      'Blank subtype abundance cells and explicit numeric zeros both denote nondetection; blanks are encoded as zero.'
    else 'Non-default blank policy selected: consult cohort_audit/missing_value_policy.csv.'),file.path(results,'READ_ME_FIRST.txt'))
  invisible(cohort)
}

if(sys.nframe() == 0L && !identical(Sys.getenv('FIG2_FUNCTIONS_ONLY','0'),'1')) run_fig2()
