...
load_metadata <- function(csv_path) {
  ...
  if (any(depth < 0, na.rm = TRUE)) stop("Clean_Reads_GBp contains negative values.")
...
load_subtype_matrix <- function(subtype_dir, basename_map = c(
    Hospital = "hospital_subtypes.xlsx",
    WWTP = "wwtp_subtypes.xlsx",
    Community = "community_subtypes.xlsx",
    `Wet Market` = "wet_market_subtypes.xlsx"
  )) {
  ...
  for (setting in names(files)) {
    ...
    for (sheet in readxl::excel_sheets(fp)) {
      ...
      if (any(!is.finite(numeric_vals))) stop("Nonfinite abundance in: ", basename(fp), " / ", sheet)
...
compute_accumulation <- function(mat, meta, n_perm = 300L) {
...
compute_pcoa <- function(mat, meta, test_permutations = 999L, seed = 123L) {
  meta <- align_metadata(mat, meta)
  if (!is.numeric(mat) || any(!is.finite(mat)) || any(mat < 0)) {
    stop("PCoA requires a finite, nonnegative numeric matrix.")
  }
  ...
  test_permutations <- as.integer(test_permutations)
  if (length(test_permutations) != 1L || is.na(test_permutations) || test_permutations < 1L)
    stop("test_permutations must be a positive integer.")
  meta$Sample_Type <- droplevels(factor(meta$Sample_Type))
  if (nlevels(meta$Sample_Type) < 2L || any(table(meta$Sample_Type) < 2L))
    stop("PERMANOVA and PERMDISP require at least two settings and two samples per setting.")
  set.seed(seed)
  perm <- vegan::adonis2(dist_mat ~ Sample_Type, data = meta, permutations = test_permutations)
  dispersion_model <- vegan::betadisper(dist_mat, meta$Sample_Type)
  set.seed(seed)
  dispersion_test <- vegan::permutest(dispersion_model, permutations = test_permutations)
  test_table <- function(x) {
    out <- data.frame(Term = rownames(x), as.data.frame(x), row.names = NULL, check.names = FALSE)
    names(out)[names(out) == "Pr(>F)"] <- "P"
    out
  }
  permanova <- test_table(perm)
  permanova$Term[permanova$Term == "Model"] <- "Sample_Type"
  ...
  list(scores = scores,
       ...
       permanova = permanova, dispersion = test_table(dispersion_test$tab),
...
parse_cli <- function(args) {
  defaults <- list(
    metadata = "data/fig1/sample_metadata.csv",
    subtype_dir = "data/subtypes",
    output = file.path(getwd(), "Fig1_b-e.pdf"),
    n_perm = 300L,
    test_permutations = 999L
  )
  ...
  defaults$test_permutations <- as.integer(defaults$test_permutations)
  if (is.na(defaults$test_permutations) || defaults$test_permutations < 1L)
    stop("--test-permutations must be a positive integer.")
  defaults
}
...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  pcoa <- compute_pcoa(mat, meta_for_matrix, cfg$test_permutations)
  ...
  prefix <- tools::file_path_sans_ext(cfg$output)
  readr::write_tsv(pcoa$permanova, paste0(prefix, "_PERMANOVA.tsv"))
  readr::write_tsv(pcoa$dispersion, paste0(prefix, "_PERMDISP.tsv"))
